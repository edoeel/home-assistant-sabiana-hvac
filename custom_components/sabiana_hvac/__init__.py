...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        ...
        _LOGGER.info("Successfully setup Sabiana HVAC integration for entry %s", entry.entry_id)
        return True
    except Exception as err:
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        ...
        if unload_ok:
            ...
            _LOGGER.info("Successfully unloaded Sabiana HVAC integration for entry %s", entry.entry_id)
        else:
            _LOGGER.warning("Failed to unload some platforms for entry %s", entry.entry_id)
        
        return unload_ok
    except Exception as err:
        _LOGGER.error("Error unloading Sabiana HVAC integration for entry %s: %s", entry.entry_id, err)
        return False
